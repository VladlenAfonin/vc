...
PROVER_DATA = "./benches/results/data/folding-factor-prover.txt"
VERIFIER_DATA = "./benches/results/data/folding-factor-verifier.txt"
DEGREE_DATA = "./benches/results/data/folding-factor-factors.txt"
FIGURE = "./benches/results/fig/folding-factor.pdf"
...
def main() -> int:
    ...
    if not args.skip:
        ...
        numpy.savetxt(DEGREE_DATA, ff_logs)
    else:
        prover_times = numpy.loadtxt(PROVER_DATA)
        verifier_times = numpy.loadtxt(VERIFIER_DATA)
        ff_logs = numpy.loadtxt(DEGREE_DATA)

    plt.style.use(["science", "russian-font"])

    fig, ax = plt.subplots()
    ax.set_yscale("log")
    ax.set_xlabel("Фактор фолдинга")
    ax.set_ylabel("Время выполнения, мс")
    ax.plot(ff_logs, prover_times, label="Доказывающий")
    ax.plot(ff_logs, verifier_times, label="Проверяющий")
    ax.legend()
